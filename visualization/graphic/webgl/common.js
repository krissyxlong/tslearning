...
function initSlider (n, func, range) {
  const [min, max] = range;
  ...
  document.getElementById(sliderName).addEventListener('dragover', (e) => {
    ...
    t = setTimeout(() => {
      ...
      const value = min + (max - min) * (left / 113);
      func(value);
      clearTimeout(t);
      t = null;
    }, 20);
...
function debounce (func, timer) {
  ...
  return function () {
    ...
    t = setTimeout(function () {
      func.apply(his, arguments);
      clearTimeout(t);
    }, timer);
  };
}

function slider1 (sliders) {
  sliders.forEach((s, index) => {
    const { func, range } = s;
    initSlider(index + 1, func, range);
  });
}
...
var m4 = {
  ...
  /**
   * 
   * @param {number} fieldOfViewInRadians ：视场角，类似于相机光圈
   * @param {number} aspect ：X 通过乘以 aspect 获取
   * @param {number} near ：决定正面被切割的位置，对应 Z = -1
   * @param {number} far ：决定背面被切割的位置，对应 Z = 1
   * @returns 
   */
...
function createAndSetupTexture (gl) {
  ...
  // 参数：指定绑定点目标为二维纹理、纹理坐标水平填充/垂直填充
  // 参数：指定绑定点目标为二维纹理、纹理缩小滤波器/放大滤波器
...
// 启动 F 位置
function startFposition (gl, positionBuffer, positionLocation) {
...
// 计算映射矩阵
function getProjectionMatrix (gl, fieldOfViewRadians) {
